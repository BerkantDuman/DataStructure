...
int aynimi(struct hash_tablosu *h1, struct hash_tablosu *h2){
    ...
    for (int i=0;i<h1->tablo_uzunlugu+1;i++){
        if ((h1->tablo_basi+i)->counter != (h2->tablo_basi+i)->counter) return 0;
        else{
            if(h1->tablo_basi->counter !=0){
                while(h1->tablo_basi->header !=NULL){
                    if(h1->tablo_basi->header->anahtar != h2->tablo_basi->header->anahtar) return 0;
                    else{
                        h1->tablo_basi->header=h1->tablo_basi->header->next;
                        h2->tablo_basi->header=h2->tablo_basi->header->next;
                    }
                }
            }
...
int main(int argc, char** argv) {
   ...
   // print_hash_table(htable); 
    ...
    insert_hash_table(htable,"almanya");
    insert_hash_table(htable,"berkant");
    insert_hash_table(htable,"mahmut");
...
//   print_hash_table(htable);
    ...
    insert_hash_table(htable2,"almanya");
    insert_hash_table(htable2,"berkant");
    insert_hash_table(htable2,"mahmut");
    insert_hash_table(htable2,"Berkant");
    
    
   
   // print_hash_table(htable2);
    delete_hash_table(htable2,"besiktas");
    //print_hash_table(htable2);
    
    delete_hash_table(htable2,"besiktas");
   // print_hash_table(htable2);
    
    hash_table_buyut(&htable2, 7,13);
    print_hash_table(htable2);
